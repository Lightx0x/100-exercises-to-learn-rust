    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(run_listener(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(run_listener(second, reply));
...
async fn run_listener<T>(listener: TcpListener, reply: Arc<T>)
where 
    T: Display + Send + Sync + 'static,
{
    loop {
